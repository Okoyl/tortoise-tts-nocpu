...
def classify_audio_clip(clip):
    ...
    classifier.load_state_dict(torch.load(get_model_path(
        'classifier.pth'), map_location=torch.device('cuda')))
...
class TextToSpeech:
    ...
    def __init__(self, autoregressive_batch_size=None, models_dir=MODELS_DIR, enable_redaction=True, device=None):
        ...
        if os.path.exists(f'{models_dir}/autoregressive.ptt'):
            ...
            self.autoregressive = UnifiedVoice(max_mel_tokens=604, max_text_tokens=402, max_conditioning_inputs=2, layers=30,
                                               ...
                                               train_solo_embeddings=False).eval()  # .cpu().eval()
            ...
            self.diffusion = DiffusionTts(model_channels=1024, num_layers=10, in_channels=100, out_channels=200,
                                          ...
                                          layer_drop=0, unconditioned_percentage=0).eval()  # .cpu().eval()
        ...
        self.clvp = CLVP(dim_text=768, dim_speech=768, dim_latent=768, num_text_tokens=256, text_enc_depth=20,
                         ...
                         use_xformers=True).eval()  # .cpu().eval()
    ...
    def load_cvvp(self):
        ...
        self.cvvp = CVVP(model_dim=512, transformer_heads=8, dropout=0, mel_codes=8192, conditioning_enc_depth=8, cond_mask_percentage=0,
                         speech_enc_depth=8, speech_mask_percentage=0, latent_multiplier=1).eval()  # .cpu().eval()
    ...
    def get_conditioning_latents(self, voice_samples, return_mels=False):
        ...
        with torch.no_grad():
            ...
            auto_conds = torch.stack(auto_conds, dim=1).to(self.device)
    ...
    def get_random_conditioning_latents(self):
        ...
        if self.rlg_auto is None:
            ...
            self.rlg_auto.load_state_dict(torch.load(get_model_path(
                'rlg_auto.pth', self.models_dir), map_location=torch.device('cuda')))
            self.rlg_diffusion = RandomLatentConverter(2048).eval()
            self.rlg_diffusion.load_state_dict(torch.load(get_model_path(
                'rlg_diffuser.pth', self.models_dir), map_location=torch.device('cuda')))
        with torch.no_grad():
    ...
    def tts(self, text, voice_samples=None, conditioning_latents=None, k=1, verbose=True, use_deterministic_seed=None,
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(samples, disable=not verbose):
                ...
                if cvvp_amount != 1:
                    clvp = self.clvp(text_tokens.repeat(
